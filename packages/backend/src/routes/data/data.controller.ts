...
import { getFullContentForPost, getPreviewForPost } from "../../models/post";
...
const getPreview = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { html } = await getPreviewForPost(db.post.toProto(post));
...
const getFull = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { html } = await getFullContentForPost(db.post.toProto(post));
